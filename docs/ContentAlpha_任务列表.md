...
- 计划已调整：后续优先级不再是继续补每个功能细节，而是先按已确认风格图把正式 UI 壳、主界面层级、弹窗样式和组件体系做成游戏界面；再根据正式 UI 需要列出数据缺口，并把已有真实运行时数据接入。
- 低优先级系统可以先保留入口和阻塞说明；但主界面、城市详情、任命出阵、战报、事件、存档等已进入正式 UI 主流程的界面，不应继续以测试控件堆叠作为目标形态。

## 下一阶段计划：正式 UI 先行

- [x] Task 47：正式 UI 实装顺序重排
  - 状态：已完成。
  - 范围：更新 Content Alpha 执行计划，把“功能逐项补齐”改为“正式 UI 壳与组件先行、数据需求随后补齐”；明确哪些界面必须先改成正式视觉，哪些系统只保留入口。
  - 根因：UI 风格已经确认后，继续围绕测试布局补小功能会让界面长期停留在原型工具状态；正常游戏 UI 流程应先确定正式界面形态，再反推数据字段和交互缺口。
  - 边界：本任务只调整计划和执行边界，不改业务逻辑，不伪造任何 UI 所需数据。
  - 验收：任务列表、UI 风格文档、版本路线必须明确新顺序；后续任务应优先指向正式 UI 场景结构和组件，而不是低优先级功能补全。
  - 验证：
    - 已核对 `docs/ContentAlpha_任务列表.md`、`docs/UI_正式界面风格方向.md`、`docs/项目版本路线与执行边界.md` 和 `docs/项目说明.md` 均明确“正式 UI 形态先行、数据缺口随后补齐”的执行顺序。
    - 本任务仅调整计划和执行边界，不改 Godot 运行时逻辑；未运行 Godot 自动化测试。

- [x] Task 48：正式主界面视觉结构重建
  - 状态：已完成。
  - 范围：以 `docs/资源/ui_style_concepts/content_alpha_formal_ui_style_v1.png` 为基准，重建 `FormalHud` 的正式游戏界面结构：顶部状态、主地图信息层、右侧详情、左下人物/势力卡、底部命令区和弹窗入口层级。
  - 根因：Task 42 已经把 HUD 初步重排为正式骨架，但地图中央缺少正式信息层，任命出阵、战报、事件和存档弹窗仍直接挂在 HUD 根节点；继续这样扩展会让主界面层级和弹窗入口混在一起，后续组件统一困难。
  - 数据原则：只接已有真实 `runtime_state` 字段；正式 UI 需要但当前没有的数据必须列为数据缺口，不用测试 ID、默认值或假字段填充。
  - 验收：界面第一眼应是正式游戏 HUD，而不是测试面板；原有地图点击、城市详情、出阵、推进、战报、事件、存档入口不能断。
  - 边界：本任务只重建 HUD 显示层级，不新增正式地图坐标、正式小地图、州郡边界、正式武将名册或新业务规则；地图摘要只读取 `runtime_state.cities / routes / armies`，缺 `route_type / blocks_enemy_passage / state` 等字段必须失败。
  - 副作用：弹窗节点路径从 HUD 根节点移动到 `PopupLayer` 下；已同步脚本和测试路径，外部仍通过 Formal HUD 方法打开面板。
  - 验证：
    - `test_formal_hud.gd` 通过，确认主地图信息层、弹窗入口层和原有命令入口存在，并读取真实运行时状态。
    - `test_main_content_alpha_workbench.gd` 通过，确认主场景地图选择和推进结算仍能驱动 Formal HUD。
...
- [x] Task 50：正式城市与势力信息 UI 重构
  - 状态：已完成。
  - 范围：按正式界面结构重做城市详情、势力摘要、治理状态和城市操作区；先定义 UI 需要的字段，再对照运行时状态列出缺失项。
  - 根因：城市详情虽然已接入真实状态，但仍主要是兵粮/治理文本块，缺少正式 UI 所需的势力摘要、分区顺序和数据缺口披露；继续直接扩展容易把不存在的上限、产量、建筑显示成假字段。
  - 数据原则：兵、粮、民心、治安、士族、太守、属官等来自真实状态；不存在的上限、收益、建筑、资源产量不得伪造。
  - 验收：城市详情能作为正式游戏面板使用，并能明确暴露数据缺口。
  - 边界：本任务只重构城市详情与势力摘要信息结构，不新增正式城市主数据、不推导兵力/粮草上限、不伪造资源产量或建筑列表。
  - 副作用：`CityDetailPresenter` 对势力字段新增 `legitimacy / prestige` 校验；缺少势力正统/名望时城市详情会失败暴露，而不是显示空摘要。
  - 验证：
    - 新增 `test_city_detail_formal_ui.gd`，先确认旧实现缺少正式分区、缺口输出和 Panel 缺口节点，再验证正式分区、势力摘要和缺口渲染。
    - `test_formal_hud.gd` 通过，确认右侧城市详情、任命和出阵入口未被重构破坏。
    - `test_main_content_alpha_workbench.gd` 通过，确认主场景选择城市仍能刷新 Formal HUD。
    - `test_formal_ui_component_factory.gd` 通过，确认动态组件工厂仍可用。

- [ ] Task 51：正式任命出阵弹窗重构
  - 范围：先按正式 UI 风格重做任命/出阵弹窗结构，再接入真实武将候选、路线、兵粮和确认反馈；路线预览、目标确认等功能放在正式弹窗形态下实现。
  - 数据原则：可用武将、路线、目标城市、消耗、阻断原因必须来自真实状态和系统校验；缺失数据直接暴露。
  - 验收：弹窗不再是基础 `OptionButton + SpinBox` 的测试排版；业务仍通过 `AppointmentSystem / SortieSystem / MarchSystem`。

- [ ] Task 52：正式反馈界面重构
  - 范围：重做战报、事件日志、存档读档的正式视觉层级和对象跳转入口；优先让反馈像正式游戏记录，而不是运行时日志列表。
  - 数据原则：战报、事件、存档摘要仍读取真实系统输出；完整历史事件链、多槽存档和版本迁移未完成前必须标记为缺口。
  - 验收：反馈界面视觉风格与主 HUD 一致，且不会掩盖缺失历史链或存档迁移能力。
