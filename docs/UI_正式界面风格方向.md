...
- 正式主界面 HUD 外壳已进入 `scenes/formal_hud.tscn`，城市详情只读面板已进入 `scenes/city_detail_panel.tscn`；后续任命出阵、战报、事件和存档界面应沿该外壳继续拆分，而不是继续堆叠调试文本。

## 仍未完成

- 授权中文 UI 字体尚未实现。
- 正式武将名册、任命出阵、战报、事件和存档界面仍处于规格规划阶段；城市详情当前只完成只读面板，任命/出阵动作仍未完成。
