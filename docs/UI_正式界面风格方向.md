...
- 当前执行顺序已调整为“正式 UI 形态先行”：风格图确认后，优先把主界面、右侧详情、底部命令、弹窗和反馈界面做成正式游戏 UI，再根据 UI 信息需求反推数据字段和系统缺口。
- 后续不得以测试控件堆叠作为已完成目标形态；`OptionButton / SpinBox / DebugPanel` 等可以用于早期验证，但进入正式 UI 主流程后必须替换为符合风格图和 Theme 的游戏界面组件。
- UI 层不得为了凑齐视觉效果生成假字段、默认值或 mock 结果；正式 UI 需要但运行时没有的数据，必须作为数据缺口记录。
- 动态创建的正式 UI 控件应优先通过 `FormalUiComponentFactory` 生成，统一读取 `ui_theme_tokens` 的字号、间距和控件约束；禁用入口必须保留真实阻塞原因，不得用“暂不可用”一类泛化文案掩盖缺口。
...
- Formal HUD 已补充主地图信息层和统一弹窗入口层：地图层只展示来自真实 `cities / routes / armies` 的城市、路线、关隘、阻断和行军摘要；任命出阵、战报、事件、存档面板统一挂载在 `PopupLayer` 下，后续正式弹窗应沿该层级扩展。
- 右侧城市详情已按正式信息栏拆成势力摘要、兵粮民生、治理状态、太守属官、城市动作和数据缺口区；不存在真实上限、产量、建筑字段的数据不得显示成伪造的“当前 / 上限”或假收益，必须进入数据缺口。
...
- 授权中文 UI 字体尚未实现。
- 正式 UI 生产化重构尚未完成；主界面、城市详情、任命出阵、战报、事件日志、存档读档虽然已有第一层场景和真实系统入口，但仍需要按风格图重建正式组件、布局层级和弹窗样式。
- 正式武将名册仍处于规格规划阶段；任命与出阵当前已接通真实系统，但正式武将选择器、路线预览、目标确认和结果反馈强化应放在正式弹窗重构后实现；战报和事件日志当前已接通真实运行时日志，但仍缺完整历史事件链和对象跳转协议；存档读档当前已接通真实 `SaveSystem`，但仍缺多槽 UI、版本迁移策略和主菜单入口。
