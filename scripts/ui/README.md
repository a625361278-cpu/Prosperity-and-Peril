...
- `CityDetailPanel` 是第一层正式城市详情面板，读取 `runtime_state.cities / forces / officers`，并按势力摘要、兵粮民生、治理状态、太守属官、动作区和数据缺口区展示；任命和出阵按钮会打开 `AppointmentSortiePanel`。不存在真实上限、产量或建筑字段时不得伪造成“当前 / 上限”或假收益，必须由 `CityDetailPresenter` 输出到 `data_gap_rows`。
