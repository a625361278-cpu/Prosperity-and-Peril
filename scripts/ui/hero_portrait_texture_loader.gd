...
static func load_texture_from_row(row: Dictionary) -> Dictionary:
	...
	if load_error != OK:
		return _failure(["hero portrait texture load failed: %s error=%d" % [source_path, load_error]])
	if image.is_empty():
		return _failure(["hero portrait texture image is empty: %s" % source_path])

	var texture := ImageTexture.create_from_image(image)
	if texture == null:
		return _failure(["hero portrait texture creation failed: %s" % source_path])
